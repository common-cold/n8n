...
export async function runAgent(node: CustomNode): Promise<NodeExecutionResult> {
    ...
    try {
        ...
        const response = await runGeminiAgent(agentParam.prompt!, credentialData.apiKey, getProperModelName(llmParam.modelName)!, toolMetadataList);
...
function getProperModelName(dbModelName: string) {
    if (dbModelName === "Gemini 2.0 Flash") {
